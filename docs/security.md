...
On Unix, state directories/files are set to `0700`/`0600`. On Windows, state directories and files are opened through retained non-reparse handles and repaired and verified through those same handles. The protected DACL must contain one allow ACE for each required unique binary SID: normally the current SID, `SYSTEM`, and built-in Administrators, or only `SYSTEM` and built-in Administrators when the process runs as LocalSystem. Only `current user == SYSTEM` is normalized; every other role collision and every duplicate on-disk trustee fails closed. Access and inheritance flags remain exact, and repair preserves the existing owner; structural, policy, handle-identity, or post-repair verification failures fail closed. A LocalSystem state directory cannot be downgraded to an unpatched binary that expects three ACEs; that downgrade is unsupported and fails closed unless the correction is backported. This state-artifact policy is separate from the current-SID-only named-pipe and kernel-object policy.
...
Provider output, task text, repository content, Git metadata, usage-probe JSON, and subprocess output are untrusted. The design does not defend against an administrator who can replace binaries, read process memory, or rewrite the entire state directory. The Windows state DACL intentionally trusts elevated Administrators and `SYSTEM`; it does not protect state from those principals. The `DEBUG_PROCESS` QA audit observes only the controlled child tree launched by the harness. It is evidence about that tree, not a host-wide monitor or a defense against privileged processes. Use endpoint protection, code-signing policy, repository access controls, and backup retention appropriate to the source classification.
