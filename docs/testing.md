...
No provider credentials are needed. If an integration test asks for a provider login or consumes Enterprise quota, stop: that test violates the repository contract. For lower disk pressure, the repository verification suite may be run with `CARGO_INCREMENTAL=0`.
